...
const RACES = yamlload("../data/character-races.yml").races
const CLASSES = yamlload("../data/character-classes.yml").classes
...
const cclass = (chain) => {
  return {
    valid(cclass) {
      return _.get(CLASSES, cclass) != null
    },
    set(cclass) {
      if (!this.valid(cclass)) {
        throw new Error(`Invalid class: ${cclass}`)
      }
      chain.character.class = _.get(CLASSES, cclass)
      return chain
    },
    modify() {
    ...
    assert(cclass) {
      ...
      const expected = _.get(CLASSES, cclass)
      const message = `Expected class to be ${cclass}`
      assert.deepEqual(actual, expected, message)
      return chain
    },
  }
...
module.exports = (character) => {
  let chain = {}
  chain.character = character || create()
