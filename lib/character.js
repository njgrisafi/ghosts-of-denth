...
character.class = {
  all: _.memoize(() => yamlload("characters/classes.yml").classes),
  byKey: _.memoize((key) => _.get(self.class.all(), key)),
  valid: _.memoize((cclass) => self.class.byKey(cclass) != null),
  set: curry((cclass, character) => {
    if (!self.class.valid(cclass)) {
  ...
  assert: curry((cclass, character) => {
    const actual = self.class.byKey(character.class)
    const expected = self.class.byKey(cclass)
...
const attribute = (attribute) => {
  ...
  module.modifiers = {
    derive: (character) => {
      ...
      const cclass = self.class.byKey(character.class)
